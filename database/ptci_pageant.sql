...
(1, 1, 27, 90.00, 89.00, 88.00, 87.00, '2025-10-06 08:59:01');
...
ALTER TABLE `final_score`
  ADD PRIMARY KEY (`id`),
  ADD UNIQUE KEY `unique_cand` (`cand_id`);
...
--
ALTER TABLE `final_score`
  ADD CONSTRAINT `final_score_ibfk_1` FOREIGN KEY (`cand_id`) REFERENCES `contestants` (`cand_id`) ON DELETE CASCADE;

--
-- Table structure for table `overall_total_score`
--

CREATE TABLE `overall_total_score` (
  `id` int(11) NOT NULL,
  `user_id` int(11) DEFAULT NULL,
  `cand_id` int(11) NOT NULL,
  `talent` decimal(5,2) DEFAULT 0.00,
  `production_number` decimal(5,2) DEFAULT 0.00,
  `swimwear` decimal(5,2) DEFAULT 0.00,
  `uniform` decimal(5,2) DEFAULT 0.00,
  `formal` decimal(5,2) DEFAULT 0.00,
  `total_score` decimal(6,2) GENERATED ALWAYS AS (`talent` + `production_number` + `swimwear` + `uniform` + `formal`) STORED,
  `created_at` timestamp NOT NULL DEFAULT current_timestamp(),
  `updated_at` timestamp NOT NULL DEFAULT current_timestamp() ON UPDATE current_timestamp()
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

--
-- Indexes for table `overall_total_score`
--
ALTER TABLE `overall_total_score`
  ADD PRIMARY KEY (`id`),
  ADD UNIQUE KEY `unique_cand` (`cand_id`);

--
-- AUTO_INCREMENT for table `overall_total_score`
--
ALTER TABLE `overall_total_score`
  MODIFY `id` int(11) NOT NULL AUTO_INCREMENT;

--
-- Constraints for table `overall_total_score`
--
ALTER TABLE `overall_total_score`
  ADD CONSTRAINT `overall_total_score_ibfk_1` FOREIGN KEY (`cand_id`) REFERENCES `contestants` (`cand_id`) ON DELETE CASCADE;
...
ALTER TABLE `talent_score`
  ...
  ADD CONSTRAINT `talent_score_ibfk_2` FOREIGN KEY (`judge_id`) REFERENCES `users` (`id`) ON DELETE CASCADE;
COMMIT;
